...
export async function GET(req: NextRequest) {
  ...
  if (!agentId) {
    return NextResponse.json({
      configured: false,
      next: 'Create an ElevenLabs Agent for Ava and add ELEVENLABS_AGENT_ID to Vercel.'
    });
  }

  try {
    const response = await fetch(`${ELEVENLABS_API}/convai/conversation/get-signed-url?agent_id=${encodeURIComponent(agentId)}`, {
