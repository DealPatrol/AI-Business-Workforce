...
async function createAvaCheckout(req: NextRequest, planKey: PlanKey) {
  ...
  params.set('success_url', `${origin}/onboarding/ava?session_id={CHECKOUT_SESSION_ID}&plan=${planKey}`);
  ...
  params.set('line_items[0][price_data][product_data][description]', `${plan.minutes} included voice minutes/month; overage $${(plan.overage / 100).toFixed(2)}/minute.`);
  ...
  if (!stripe.ok || !data.url) {
    ...
    return { error: NextResponse.json({ error: data?.error?.message || 'Could not start Ava checkout.' }, { status: 500 }) };
  }
