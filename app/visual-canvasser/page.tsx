...
export default function VisualCanvasserSales(){
 ...
 return <main className="vc-page">
  ...
  <section id="example" className="section wrap"><div className="sectionhead"><span>INTERACTIVE EXAMPLE</span><h2>One engine. Different home-service businesses.</h2><p>Choose a service to see the type of property-specific proposal the campaign could create.</p></div><div className="vc-tabs">{industries.map(x=><button key={x} onClick={()=>setIndustry(x)} className={industry===x?'active':''}>{x}</button>)}</div><div className="vc-demo"><div className="vc-before"><small>PROPERTY TODAY</small><div className="vc-service-image-frame"><Image className="vc-service-image" src={serviceImages[industry]} alt={`Current property view for ${industry.toLowerCase()}`} width={1200} height={700} priority={industry === 'Landscaping'}/></div><b>Current property view</b><p>Authorized/current imagery is used when available. We do not represent unavailable property imagery as live.</p></div><div className="vc-arrow"><ArrowRight/></div><div className="vc-after"><small>{industry.toUpperCase()} CONCEPT</small><div className="vc-service-image-frame"><Image className="vc-service-image" src={serviceImages[industry]} alt={`Realistic proposed ${industry.toLowerCase()} improvement`} width={1200} height={700}/></div><b>Realistic proposed improvement</b><ul>{examples[industry].map(x=><li key={x}><Check size={15}/>{x}</li>)}</ul></div><div className="vc-scope"><small>WHAT THE HOMEOWNER SEES</small><h3>Your personalized project idea</h3><p>A concise explanation of the visible changes, plus a QR code to explore the project and request an estimate.</p><div className="vc-qr big"><QrCode size={70}/></div><button className="button">Request My Estimate</button></div></div></section>
